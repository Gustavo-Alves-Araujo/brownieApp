  <template>
    <div class="flex min-h-screen">
      ...
      <aside class="w-64 bg-gray-800 text-white flex flex-col p-4">
        ...
        <ul class="space-y-4">
          <!-- Dashboard -->
          <li class="flex items-center space-x-2 hover:text-gray-300 cursor-pointer">
          ...
          <li>
            <div @click="toggleTasks" class="flex items-center space-x-2 hover:text-gray-300 cursor-pointer">
              <ClipboardListIcon class="w-6 h-6" />
              <span>Tarefas</span>
              <ChevronDownIcon class="w-5 h-5 transform transition-transform" :class="{ 'rotate-180': isTasksOpen }" />
            </div>
      ...
      <div class="flex flex-col w-full">
        <!-- Header -->
        <header class="flex items-center justify-between bg-gray-700 text-white p-4">
          <div></div>
          <h1> Saldo: {{ users.find(user => user.name == selected)?.coins }}</h1>
          <USelectMenu class="" placeholder="Select a person" v-model="selected"
...
watch(selected, (newValue) => {
  const user = users.value.find((user) => user.name === newValue);
